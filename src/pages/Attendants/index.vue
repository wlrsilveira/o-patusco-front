          ...
          <v-row>
            <v-col class="d-flex justify-end mt-3">
              <v-btn
                ...
                color="primary"
                rounded
                @click="$router.push('/attendants/create')"
              >
                Novo Atendente
                <v-icon class="ml-2">
...
export default {
  ...
  methods: {
    ...mapActions({
      setAttendant: SET_USER,
      setAttendantDialog: SET_USER_DIALOG,
      setReadOnly: SET_READ_ONLY,
      fetchUsers: FETCH_USERS
    }),
    viewUser(item, readOnly = true) {
      this.setAttendant(item);
      this.setAttendantDialog(true);
      ...
      this.$swal.fire({
        ...
        if (result.isConfirmed) {
          api.delete(`/attendant/${item.id}`)
            .then(() => {
              this.fetchUsers({namespace: 'attendant'});
              this.$swal.fire({
                title: "Deletado!",
